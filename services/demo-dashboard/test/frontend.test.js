...
test('company detail provides seven CRM tabs and keeps Customer Match separate from DPV Score', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  const html = await readFile(new URL('../public/index.html', import.meta.url), 'utf8');
  for (const tab of ['overview','evidence','contacts','social','matching','scoring','history']) assert.equal(app.includes(`['${tab}'`),true);
...
test('opportunity table uses the combined endpoint and retains the lead-directory fallback', async () => {
  const app = await readFile(new URL('../public/app.js', import.meta.url), 'utf8');
  assert.match(app,/json\('\/api\/opportunities\?limit=100'\)/);
...
test('Phase 5 V2.3 keeps management and Mexico historical matches separate', async () => {
  ...
  assert.match(html,/id="opportunity-table"[\s\S]*colspan="12"/);
...
test('company-facing frontend copy passes repository wording audit', async () => {
  const files = await Promise.all(['index.html','app.js','crm-shell.js'].map(name=>readFile(new URL(`../public/${name}`,import.meta.url),'utf8')));
