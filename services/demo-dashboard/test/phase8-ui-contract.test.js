...
test('Opportunities renders exactly seven default decision columns and a useful true zero state', async () => {
  const [html, app] = await readMany(['index.html', 'app.js']);
  ...
  const columnClasses = [
    'op-col-company', 'op-col-market-product', 'op-col-status', 'op-col-buyer-model',
    'op-col-product-match', 'op-col-contact', 'op-col-supplier-action'
  ];
  ...
  assert.equal((renderer.match(/<td\b/g) || []).length, 7);
