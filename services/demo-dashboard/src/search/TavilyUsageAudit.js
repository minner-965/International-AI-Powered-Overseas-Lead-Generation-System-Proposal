...
function failureStatus(error) {
  return ['TIMEOUT','NETWORK_ERROR','CREDIT_OR_RATE_LIMIT'].includes(String(error?.code || '').toUpperCase())
    ? 'TEMPORARY_ERROR'
    : 'FAILED';
}
...
export class TavilyCreditBudget {
  constructor({pool=null,runCapUnits=25,dailyCapUnits=100,billingPeriodCapUnits=1000,reservationUnits=1,now=()=>new Date()}={}) {
    this.pool=pool;
  ...
  async reserve({researchJobId,companyId=null,purpose,endpoint,request}) {
    ...
    const audit={purpose:String(purpose || '').trim().toUpperCase()};
    if(!this.pool) {
      ...
      const dailyTotal=[...this.localEvents.values()].filter(item=>new Date(item.created_at).toISOString().slice(0,10)===day)
        .reduce((sum,item)=>sum+Number(item.reserved_units||0)+Number(item.used_units||0),0);
      if(runTotal+this.reservationUnits>this.runCapUnits
        ...
        throw Object.assign(new Error('Tavily credit budget reached'),{code:'TAVILY_CREDIT_CAP'});
      }
    ...
    try {
      ...
      await client.query(`INSERT INTO leadgen.provider_credit_ledger(provider,billing_period,credit_limit_units)
        VALUES ($1,$2,$3) ON CONFLICT(provider,billing_period) DO NOTHING`,[PROVIDER,period,this.billingPeriodCapUnits]);
      ...
      if(recoverable){const used=Number(latest.reserved_units||this.reservationUnits);const recoveredStatus=Number(latest.result_payload.audit.result_count)>0?'COMPLETED':'NOT_FOUND';
        ...
        const recovered=await client.query(`UPDATE leadgen.provider_usage_events SET status=$2,used_units=$3,
          reserved_units=0,error_code=NULL,completed_at=now() WHERE id=$1 RETURNING *`,[latest.id,recoveredStatus,used]);
      ...
      if(!current||Number(run.rows[0].total)+this.reservationUnits>this.runCapUnits
      ...
      const inserted=latest?.status==='TEMPORARY_ERROR'
        ?await client.query(`UPDATE leadgen.provider_usage_events SET status='RESERVED',reserved_units=$2,used_units=0,
          ...
          result_payload=$4::jsonb,created_at=now(),completed_at=NULL WHERE id=$1 RETURNING *`,[
          latest.id,this.reservationUnits,effectiveLimit-Number(current.used_units)-Number(current.reserved_units),JSON.stringify({audit})])
        :await client.query(`INSERT INTO leadgen.provider_usage_events
        (research_job_id,company_id,provider,billing_period,endpoint,request_fingerprint,status,
         reserved_units,credits_before_units,result_payload)
        VALUES($1,$2,$3,$4,$5,$6,'RESERVED',$7,$8,$9::jsonb) RETURNING *`,[
        researchJobId,companyId,PROVIDER,period,endpoint,requestFingerprint,this.reservationUnits,
        effectiveLimit-Number(current.used_units)-Number(current.reserved_units),JSON.stringify({audit})]);
  ...
  async settle(event,{usedUnits=0,status='COMPLETED',providerRequestId=null,errorCode=null,resultCount=0,referenceIds=[]}={}) {
    ...
    if(!this.pool) {
      ...
      const settled={...event,status,reserved_units:0,used_units:used,provider_request_id:providerRequestId,
        error_code:errorCode,result_payload:resultPayload,
        credits_after_units:this.billingPeriodCapUnits-this.localUsed,completed_at:this.now()};
    ...
    try {
      ...
      const result=await client.query(`UPDATE leadgen.provider_usage_events SET status=$2,reserved_units=0,
        used_units=$3,credits_after_units=$4,provider_request_id=$5,error_code=$6,
...
export class TavilyUsageAudit {
  constructor({provider,pool=null,budget=null,runCapUnits=25,dailyCapUnits=100,billingPeriodCapUnits=1000,reservationUnits=1}={}) {
  ...
  async search({researchJobId,companyId=null,purpose,request,persistResults=null,loadPersistedResults=null}) {
    if(String(this.provider?.name||'').toLowerCase()!=='tavily')return this.provider.search(request);
    let reservation=await this.budget.reserve({researchJobId,companyId,purpose,endpoint:this.endpoint,request});
