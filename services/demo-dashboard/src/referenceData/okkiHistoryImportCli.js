...
import pg from 'pg';
import { OkkiHistoryService } from './okkiHistoryService.js';

const bundlePath = process.argv[2];
const action = process.argv[3] || 'dry-run';
if (!bundlePath || !['dry-run','commit','both'].includes(action)) {
  throw new Error('Usage: node okkiHistoryImportCli.js BUNDLE_PATH [dry-run|commit|both]');
}
...
const service = new OkkiHistoryService({ pool });
try {
  const result = {};
